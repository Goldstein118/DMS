...
if (!isset($data['kategori_id'])) {
    ...
    echo json_encode(["error" => "Kategori ID tidak ditemukan"]);
    exit;
}
$kategori_id = trim($data['kategori_id']);

    $stmt = $conn->prepare("DELETE FROM tb_kategori WHERE kategori_id = ?");
    $stmt->bind_param("s", $kategori_id);
    $execute= $stmt->execute(); 
    
    if($execute&&$stmt->affected_rows>0){
    http_response_code(200);
    echo json_encode(["message" => "Kategori berhasil terhapus"]);
    }
    else {
        http_response_code(400);
    ...
    if ($e->getCode() == 1451) {
        ...
        $errorMsg = "Kategori tidak dapat dihapus karena masih digunakan";
        if ($table) {
