...
if (isset($data['table']) && $data['table'] === 'detail_invoice' && isset($data['invoice_id'])) {
    ...
    echo json_encode(["data" => $detail_data]);
    exit;
} else if (isset($data['invoice_id']) && isset($data['select']) && $data['select'] === 'select') {
    $invoice_id = trim($data['invoice_id']);
    $sql = "SELECT * FROM tb_invoice WHERE invoice_id=?";
    $stmt = $conn->prepare($sql);
    if ($stmt == false) {
        http_response_code(500);
        echo json_encode(["error" => "Failed to prepare statement: " . $conn->error]);
...
} else if (isset($data['select']) && $data['select'] === 'select_retur_pembelian') {
    ...
    WHERE i.invoice_id NOT IN (
        SELECT invoice_id FROM tb_retur_pembelian
    )";
