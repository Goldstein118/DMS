...
if (strlen($search) >= 3 && $search !== '') {
    ...
    $stmt->execute();
    $result = $stmt->get_result();
} else if (isset($data['select']) && isset($data['produk_id'])) {
    $produk_id = $data['produk_id'];
    $sql = "SELECT brand_id , produk_id FROM tb_produk WHERE produk_id=?";
    $stmt = $conn->prepare($sql);
    if ($stmt == false) {
        http_response_code(500);
...
if ($result) {
    $produk_data = [];
    $include_produk_link = !(isset($data['select']));

    while ($row = mysqli_fetch_assoc($result)) {
        if ($include_produk_link && isset($row['gambar_produk_id'])) {
            $gambar_url = getImageUrl($row['gambar_produk_id'], $conn, $upload_dir, $base_url);
