...
try {
    ...
    if (isset($data['tanggal_pengiriman']) && $data['status'] === "pengiriman") {
        ...
        $oldDataStmt = $conn->prepare("SELECT tanggal_pengiriman,no_pengiriman,status FROM tb_pembelian WHERE pembelian_id = ?");
        ...
        if (isset($oldData['tanggal_pengiriman'])) {
            $stmt_history_before = $conn->prepare("UPDATE tb_pembelian_history SET tanggal_pengiriman_before = ?,  no_pengiriman_before = ? ,status_before =? WHERE pembelian_history_id = ?");
            $stmt_history_before->bind_param(
                "ssss",
            ...
            $stmt_history_before->execute();
            $stmt_history_before->close();
        }
    ...
    if (isset($data['tanggal_terima']) && $data['status'] === "terima") {
        ...
        $oldDataStmt = $conn->prepare("SELECT tanggal_terima,status FROM tb_pembelian WHERE pembelian_id = ?");
        $oldDataStmt->bind_param("s", $pembelian_id);
        $oldDataStmt->execute();
        $oldDataResult = $oldDataStmt->get_result();
        $oldData = $oldDataResult->fetch_assoc();
        ...
        if (isset($oldData['tanggal_terima'])) {
            $stmt_history_before = $conn->prepare("UPDATE tb_pembelian_history SET tanggal_terima_before = ?,  status_before =? WHERE pembelian_history_id = ?");
            $stmt_history_before->bind_param(
                "sss",
    ...
    if (isset($data['pembelian_id']) && isset($data['tanggal_po'])) {
        ...
        $stmt = $conn->prepare("UPDATE tb_pembelian SET tanggal_po =?,supplier_id=?,gudang_id=?,keterangan=?,ppn=?,diskon=?,nominal_pph=?,status=?
        WHERE pembelian_id = ?");
        $stmt->bind_param(
            "ssssdddss",
            $tanggal_po,
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        ...
        if (isset($data['biaya_tambahan'])) {
            ...
            while ($row = $existingTambahanResult->fetch_assoc()) {
                ...
                executeInsert(
                    $conn,
                    "INSERT INTO tb_biaya_tambahan_history (biaya_tambahan_history_id, pembelian_history_id, data_biaya_id, keterangan, jlh,urutan, tipe_biaya_tambahan_history)
                         VALUES (?,?,?,?,?,?,?)",
                    [
